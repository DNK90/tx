...
import {
  ...
  isLegacyTxData, isSponsoredTxData,
} from './types.js'
...
export class TransactionFactory {
  ...
  public static fromTxData<T extends TransactionType>(
    ...
    if (!('type' in txData) || txData.type === undefined) {
      ...
      } else if (isSponsoredTxData(txData)) {
        return SponsoredTransaction.fromTxData(txData, txOptions) as Transaction[T]
      } else {
  ...
  public static fromSerializedData<T extends TransactionType>(
    ...
    if (data[0] <= 0x7f) {
      ...
      switch (data[0]) {
        ...
        case TransactionType.Sponsored:
          return SponsoredTransaction.fromSerializedTx(data, txOptions) as Transaction[T]
        default:
